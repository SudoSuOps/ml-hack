...
the README so the embedded iframe in ml-hack renders without a login click —
...
Embedded trackio dashboard for ml-hack runs.
"""

_REQUIREMENTS = "trackio\n"
_APP_PY = "import trackio\ntrackio.show()\n"

# ml-hack brand mark surfaced inside the trackio dashboard. Trackio reads
...
# copy on the swarmandbee/ml-hack Space repo so any seeded dashboard inherits
# the ml-hack branding without each user having to host the asset.
_LOGO_URL = (
    "https://huggingface.co/spaces/swarmandbee/ml-hack/"
    ...
    if _already_seeded(api, space_id):
        ...
        for path_in_repo, content in _FILES.items():
            api.upload_file(
                ...
                repo_id=space_id,
                repo_type="space",
                commit_message=f"ml-hack: seed trackio dashboard ({path_in_repo})",
            )
        seeded_files = True
