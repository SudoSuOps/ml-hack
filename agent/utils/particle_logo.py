"""Particle coalesce effect for the SWARM AND BEE ML-HACK logo.

Random particles swirl in from the edges, converge to form the text
"SWARM AND BEE / ML-HACK", hold briefly, then the final frame is printed.
...
def run_particle_logo(console: Console, hold_seconds: float = 1.5) -> None:
    ...
    text_pixels_line1 = text_to_pixels("SWARM AND BEE", scale=2)
    text_pixels_line2 = text_to_pixels("ML-HACK", scale=2)
