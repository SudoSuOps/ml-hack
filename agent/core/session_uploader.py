...
* ``row`` — single-line JSONL row used by the existing org telemetry/KPI
  pipeline (``swarmandbee/ml-hack-sessions``). Compatible with
...
* ``claude_code`` — one event per line in the Claude Code JSONL schema,
  ...
  per-user private dataset (default ``{hf_user}/ml-hack-sessions``).
"""
...
_PERSONAL_TOKEN_ENV = "_ML_HACK_PERSONAL_TOKEN"
...
def dataset_card_readme(repo_id: str) -> str:
    """Dataset card for personal ML-Hack session trace repos."""
    return """---
pretty_name: "ML-Hack Session Traces"
language:
- en
license: other
task_categories:
...
# ML-Hack session traces

This dataset contains ML-Hack coding agent session traces uploaded from local
ML-Hack runs. The traces are stored as JSON Lines files under `sessions/`,
with one file per session.

## Links

- ML-Hack demo: https://swarmandbee-ml-hack.hf.space
- ML-Hack CLI: https://github.com/huggingface/ml-hack

## Data description

Each `*.jsonl` file contains a single ML-Hack session converted to a
...
ML-Hack applies automated best-effort scrubbing for common secret patterns
