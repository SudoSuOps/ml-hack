...
def test_dataset_card_readme_has_metadata_and_public_warning():
    readme = dataset_card_readme("lewtun/ml-hack-sessions")

    assert readme.startswith("---\n")
    assert 'pretty_name: "ML-Hack Session Traces"' in readme
    assert "task_categories:\n- text-generation" in readme
    assert "- agent-traces" in readme
    assert "- coding-agent" in readme
    assert "- ml-hack" in readme
    assert 'path: "sessions/**/*.jsonl"' in readme
    assert "ML-Hack demo: https://swarmandbee-ml-hack.hf.space" in readme
    assert "ML-Hack CLI: https://github.com/huggingface/ml-hack" in readme
...
def test_upload_dataset_card_only_for_claude_code_format():
    ...
    _upload_dataset_card(api, "lewtun/ml-hack-sessions", "hf_token", "row")
    assert api.calls == []

    _upload_dataset_card(api, "lewtun/ml-hack-sessions", "hf_token", "claude_code")
    assert len(api.calls) == 1
    assert api.calls[0]["path_in_repo"] == "README.md"
    assert api.calls[0]["repo_id"] == "lewtun/ml-hack-sessions"
...
def test_update_upload_status_preserves_other_uploader_fields(tmp_path):
    ...
    _update_upload_status(
        ...
        "personal_upload_url",
        "success",
        "https://huggingface.co/datasets/user/ml-hack-sessions",
    )
