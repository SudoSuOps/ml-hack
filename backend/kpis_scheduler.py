...
Environment::
    ...
    KPI_SOURCE_REPO     default swarmandbee/ml-hack-sessions
    KPI_TARGET_REPO     default swarmandbee/ml-hack-kpis
    ML_HACK_KPIS_DISABLED  if truthy, the scheduler is not started
"""
...
async def _run_hour(hour_dt: datetime) -> None:
    ...
    try:
        ...
        api = HfApi()
        source = os.environ.get("KPI_SOURCE_REPO", "swarmandbee/ml-hack-sessions")
        target = os.environ.get("KPI_TARGET_REPO", "swarmandbee/ml-hack-kpis")
...
def start(backfill_hours: int = 6) -> None:
    ...
    global _scheduler
    if os.environ.get("ML_HACK_KPIS_DISABLED"):
        logger.info("kpis_scheduler: disabled via ML_HACK_KPIS_DISABLED")
        return
